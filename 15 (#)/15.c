 ...
 * The C language defines a byte to be the number of bits used by char, so one can have a system with 16-bit or 32-bit
   bytes, instead of the common 8-bit ones.
 ...
 * According to the ASCII code, char = 'A'; can also be written as char = 65; (in decimal form), char = '\101'; (in
   octal form) or char = '\x41'; (in hexadecimal form), and so on.
 * Escape sequences include \a (alert), \b (backspace), \n (newline), \t (horizontal tab), \v (vertical tab), \\
   (backslash), \' (single quote), \" (double quote), \? (question mark), etc.
 * When a character, including escape sequences, is part of a string of characters enclosed in double quotes, it should
